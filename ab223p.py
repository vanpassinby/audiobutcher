...
# Comment out next 3 lines when compiling with AB_DISABLE_LIBROSA or AB_USE_ALT_AUBIO
if not (AB_DISABLE_LIBROSA or AB_USE_ALT_AUBIO):
    from numpy import array, float32, median
    from librosa.onset import onset_strength, onset_detect

# Comment out next line when compiling without AB_USE_ALT_AUBIO
if AB_USE_ALT_AUBIO: import aubio
...
# Names
abVersion = "AudioButcher v2.2.3-p"
abVersionShort = "2.2.3.00"
abKnownVersions = ["2.1.0", "2.1.1", "2.2.0", "2.2.1.00", "2.2.2.00", "2.2.3.00"]
abDescription = """AudioButcher ver. 2.2.3 (Public Release), April 2023
...
def updFormats(init=False):
    global import_types, export_types
    if not init:
        abConfig.set("moreformats")
        abConfig.set("flacexport")
    if moreformats.get() == 1: import_types = import_types_2
    else: import_types = import_types_1
    if flacexport.get() == 1: export_types = export_types_2
    else: export_types = export_types_1
...
def text2list(_type, text):
    return list(map(_type, text.split()))
...
def dndAnalyzeFile(path):
    rempath = bool(remdndpath.get())
    ext = splitext(path)[-1].lower()
    if isdir(path) or not exists(path): pass
    elif ext==".abo": openOnsets(window, False, path, rempath, False)
    elif ext==".sto": openOnsets(window, True,  path, rempath, False)
    elif ext==".abp": run(cfgImport, path, rempath)
    else: run(genImportAudio, path, rempath)


def speedChange(sound, speed=1):
    ogfr = sound.frame_rate
    sound = sound._spawn(sound.raw_data, overrides={"frame_rate": int(ogfr * speed)})
    return sound.set_frame_rate(ogfr)

def findNearest(arr, value, mode, direction = 0):
    #mode 0 returns timestamp, mode 1 returns array index
    diff = [abs(i - value) for i in arr]
    idx = diff.index(min(diff))

    if direction == 1:
        if arr[idx] > value and idx > 0:
            idx -= 1
    if direction == 2:
        if arr[idx] < value and idx < len(arr) - 1:
            idx += 1

    if mode == 0: return arr[idx]
    else: return idx


def generateOnsets_librosa(source, sr = 22050):
...
def generateOnsets_aubio(source):
    wavpath = tempWavPath()
    window_size = 1024
    ...
    onset_func = aubio.onset(abConfig.get("onsetmethod"), window_size, hop_size)
...
def generateOnsets(source):
    if AB_USE_ALT_AUBIO: return generateOnsets_aubio(source)
    else: return generateOnsets_librosa(source)

def processOnsets(mode, _onsets, ons2=False, source=None, parent=None):
    global onsets, onsets2
    if _onsets!=None:
        _onsets = sorted(list(set(_onsets)))
        if mode in [0, 1]:
            try:
                if ons2: initialfile=""
...
def openOnsets(parent, ons2=False, path=None, rempath=True, window=True):
    if path==None:
        if ons2: types = onset2_types
        else: types = onset_types
        path = fd.askopenfilename(filetypes=types, initialdir=abConfig.get(defOnsPath(ons2)), parent=parent)
    if validPath(path):
        try:
            if rempath: abConfig.set(defOnsPath(ons2), path=path)
            abofile = open(path, "r")
...
def getOnsets(parent, mode, ons2=False, fromCurrent=False, path=None, window=True):
    global audio
    if fromCurrent:
        path = lastSrcPath
    elif path==None:
        path = fd.askopenfilename(filetypes=import_types, initialdir=abConfig.get("dir_srcaud"), parent=parent)
    if validPath(path):
        try:
            if window: parent.title("Manage onsets - Busy")
            if fromCurrent: _onsets = generateOnsets(audio)
            else: _onsets = generateOnsets(loadAudio(path))
            if window: parent.title("Manage onsets - Ready")
            mb.showinfo("Onsets", "All onsets detected.", parent=parent)
            processOnsets(mode, _onsets, ons2, path, parent)
        except Exception as e:
...
def eoImportOnsets(parent, entry, ons2):
    ...
    selfile = fd.askopenfilename(filetypes=types, initialdir=abConfig.get(defOnsPath(ons2)), parent=parent)
    if validPath(selfile):
        try:
...
class abConfigure:
    ...
    def set(self, what, value=0, path=""):
        values = {
            ...
            "moreformats":      moreformats.get(),
            "flacexport":       flacexport.get(),
            "remdndpath":       remdndpath.get(),
...
def abOnsetsWindow():
    ...
    if AB_USE_ALT_AUBIO: lower.grid(row=4, column=0, columnspan=3, sticky="w")
    ...
    l_method = Label(lower, text="Method: ")
    c_method = Combobox(lower, values=["energy", "hfc", "complex", "phase", "specdiff", "kl", "mkl", "specflux"], width=18)
    ...
    l_method.grid(row=0, column=0)
    c_method.grid(row=0, column=1)

    if importState != "good":
        b_geno_curr.configure(state="disabled")

    if AB_DISABLE_LIBROSA and not AB_USE_ALT_AUBIO:
        b_geno_curr.configure(state="disabled")
        b_geno_oaud.configure(state="disabled")

    cboxSel(c_andthen, abConfig.get("onsetsdropdown"))
    c_method.insert(0, abConfig.get("onsetmethod"))
    c_method.configure(state="readonly")

    c_andthen.bind("<<ComboboxSelected>>", lambda event: abConfig.set("onsetsdropdown", c_andthen.current()))
    c_method.bind("<<ComboboxSelected>>", lambda event: abConfig.set("onsetmethod", c_method.get()))
...
def abEditOnsets(parent, ons2=False):
    ...
    edonsw.wait_window()
    if closeonsets.get()==0: abOnsetsWindow()

def abError(title, exception, parent=None):
    if shadderrinf.get()==1: text = format_exc()
    else: text = exception
    mb.showerror(title, text, parent=parent)
...
def cfgEmpty():
    ...
    e_fdmode_sf.delete(0, END)
    e_fdmode_en.delete(0, END)
    e_backmaskFadeCrossfade.delete(0, END)
...
def cfgDefault():
    ...
    e_backmaskFadeCrossfade.insert(0, "-1")
    ...
    if validPath(path):
        try:
            ...
            if checkVersion(version):
                ...
                e_strtweights.insert(0, preset.get(ab, "strtweights", fallback=""))
                ...
                e_speedweights.insert(0, preset.get(ab, "speedweights", fallback=""))
                ...
                e_backmaskFadeCrossfade.insert(0, preset.get(ab, "backmaskFadeCrossfade", fallback="-1"))
...
def cfgApply():
    ...
    global fdmode_lc, fdmode_sf, fdmode_en, backmaskFadeCrossfade, minfdmsk
    ...
    backmaskFadeCrossfade = float(e_backmaskFadeCrossfade.get())
    ...
    if len(strtweights)  == 0: strtweights  = [1] * len(avgstrt)
    if len(speedweights) == 0: speedweights = [1] * len(speeds)
...
backmaskFadeCrossfade = {e_backmaskFadeCrossfade.get()}
...
def checkConfig():
    ...
    and checkComboboxes()
    and checkZeroSegment()
    and checkWrongRandom()
    and checkWrongLognorm()
    and checkWrongStarts()
    and checkWrongSpeeds()
    and checkNegativeSpeeds()
    and checkZeroFdModes()
    and checkZeroStumMethods()
    and checkOnsets())


def genImportAudio(path=None, rempath=True):
    global audio, lastSrcPath
    if path==None: path = fd.askopenfilename(filetypes=import_types, initialdir=abConfig.get("dir_srcaud"))
    if validPath(path):
        importstate("wait")
        lastSrcPath = path
        if rempath: abConfig.set("dir_srcaud", path=path)
        try:
...
def genScramble(export, duration):
    global generating, segTime, lastSeed
    def pbname():
        pb_time = strftime("%Y-%m-%d %H-%M-%S", localtime())
    ...
    good = duration!=None and (not previewing or export) and not generating
    try:
        if good: cfgApply()
        if good and checkConfig():
            ...
            if validPath(selfile):
                ...
                slicecr = genMain(audio, onsets, onsets2)
                if export:
                    if flacexport.get() == 0: slicecr.export(selfile, format="wav")
                    else: exportFlac(slicecr, selfile)
...
def genMain(audio, onsets, onsets2):
    global genReady
    global minfdmsk, trimmin, trimmax

    print("\n=== newgen ===\n")
    random.seed(lastSeed)
    ...
    if trimfile==1:
        ...
        if shiftons==1:
            onsets  = [o - trimmin for o in onsets]
            onsets2 = [o - trimmin for o in onsets2]
        lengthOfAudio = audio.duration_seconds*1000

    onsets  = [o for o in onsets  if 0 <= o <= lengthOfAudio]
    onsets2 = [o for o in onsets2 if 0 <= o <= lengthOfAudio]
    ...
    if quantizeMode == 1:
        lengthOfAudio = onsets[-1]
    elif quantizeMode == 2:
        lengthOfAudio = int(lengthOfAudio / beat_dur) * beat_dur
    ...
    while slicecr.duration_seconds < segTime:
        ...
        elif quanstrtchnce < quanstrt and quantizeMode == 1:
        ...
        def quantstart(startPosition, direction = 0):
            if quanstrtchnce < quanstrt and (not(nrmSrtChnce < normStrtChance) or quanavgstrtchnce < quanavgstrt):
                ...
                if quantizeMode == 1:
                    ...
                    if direction == 0: startPosition = round(startPosition / beat_dur) * beat_dur - spedcross
                    elif direction == 1: startPosition = int(startPosition / beat_dur) * beat_dur - spedcross
                    elif direction == 2: startPosition = math.ceil(startPosition / beat_dur) * beat_dur - spedcross
        ...
        if quanseglgthchnce < quanseglgth and (nrmSrtChnce < normStrtChance or (not(stumbchnce < stumblechance) or stumbledeviation > 0 or methStumble != 1)):
            if quantizeMode == 1:
                ...
                randdur = (math.ceil((startPosition + randdur * speed) / beat_dur) * beat_dur) - startPosition + spedcross
                while randdur <= spedcross + 0.1 and (startPosition + randdur * speed) < lengthOfAudio:
                    randdur += beat_dur
        ...
        if quanrepchce < quanrep and repmode == 1:
            if quantizeMode == 1:
            ...
            if quantizeMode == 2:
                repeatcounter = (math.ceil((startPosition + randdur + repeatcounter * speed) / beat_dur) * beat_dur) - (startPosition + randdur) + spedcross
                while repeatcounter <= spedcross and (startPosition + randdur + repeatcounter * speed) < lengthOfAudio:
                    repeatcounter += beat_dur
        ...
        if quanmaskchce < quanmask and quantizeMode > 0 and maskmode == 1:
            ...
            if quantizeMode == 2:
                maskrepeatcounter = (math.ceil((startPosition + randdurpremask + maskrepeatcounter * speed) / beat_dur) * beat_dur) - (startPosition + randdurpremask) + BackmaskCrossfade*2
                while maskrepeatcounter <= BackmaskCrossfade*2 and (startPosition + randdurpremask + maskrepeatcounter * speed) < lengthOfAudio:
                    maskrepeatcounter += beat_dur
        ...
        if chance < (100 - stopChance) or paused == 1:
            ...
            def apcrsfd(audio, audio1 = audio):
                if backmaskFadeCrossfade < 0: apfd = crossfade
                else: apfd = backmaskFadeCrossfade
                mintime = min(int(1000*audio.duration_seconds),int(1000*audio1.duration_seconds))
                if apfd > mintime:
                    apfd = round(mintime/2)
                ...
                if faderestrictchce > faderestrict or ((fadepause == 1 and not(revchance < reverseChance)) or (paused == 1 and revchance < reverseChance)):
                    if fadeout > 0:
                        if placeonsets != 0:
                            ...
                            fdsegIndex = findNearest(placeonsets, startPosition + randdur * speed, 1)
...
if True:
    ...
    export_types_1 = [["Wave", "*.wav"],
                      ["All files", "*.*"]]

    export_types_2 = [["FLAC", "*.flac"],
                      ["All files", "*.*"]]
    ...
    onset2_types = [["AudioButcher Start Onset List", "*.sto"],
    ...
    # Window
    if AB_DISABLE_TKDND2: window = Tk()
    else:
        window = TkinterDnD.Tk()
        window.drop_target_register("DND_Files")
        window.dnd_bind("<<Drop>>", dndGotFile)
    ...
    menu_file.add_command(label="Refresh file", accelerator="Ctrl+R", command=lambda: run(genImportAudio, lastSrcPath, False))
    ...
    menu_file.add_cascade(label="Preset backup", menu=smenu_pbu)
    menu_file.add_separator()
    ...
    smenu_pbu.add_command(label="Choose preset backup folder", command=lambda: updBackupPresetsDir())
    smenu_pbu.add_command(label="Open preset backup folder", command=lambda: openBackupPresetsDir())

    # Preview menu
    menu_prev.add_command(label="Preview", accelerator="Ctrl+P", command=lambda: run(genScramble, False, stp.get()))
    if not AB_DISABLE_SIMPLEAUDIO: menu_prev.add_command(label="Stop preview", accelerator="Ctrl+Alt+P", command=lambda: simpleaudio.stop_all())
    ...
    menu_pref.add_checkbutton(label="Show more import formats", var=moreformats, command=lambda: updFormats())
    menu_pref.add_checkbutton(label="Export in FLAC", var=flacexport, command=lambda: updFormats())
    menu_pref.add_checkbutton(label="Remember dropped file path", var=remdndpath, command=lambda: abConfig.set("remdndpath"))
    menu_pref.add_separator()
    ...
    if AB_DISABLE_LIBROSA and not AB_USE_ALT_AUBIO: menu_pref.entryconfig(8, state="disabled")
    ...
    window.bind("<Control-r>", lambda event: run(genImportAudio, lastSrcPath, False))
    ...
    if not AB_DISABLE_SIMPLEAUDIO: window.bind("<Control-Alt-p>", lambda event: simpleaudio.stop_all())
    ...
    lf_NoteExtension = LabelFrame(tab1, text="Note extension (Misc): ", padding=2)
    l_backmaskFadeCrossfade = Label(lf_NoteExtension, text="Backmask fade crossfade: ")
    e_backmaskFadeCrossfade = Entry(lf_NoteExtension, width=5)
    l_minfdmsk = Label(lf_NoteExtension, text="Minimum fade backmask: ")
    e_minfdmsk = Entry(lf_NoteExtension, width=5)

    lf_NoteExtension.grid(row=12, column=12, rowspan=4, columnspan=3, sticky="ws")
    l_backmaskFadeCrossfade.grid(row=0, column=0, sticky="w")
    e_backmaskFadeCrossfade.grid(row=0, column=1)
    l_minfdmsk.grid(row=1, column=0, sticky="w")
    e_minfdmsk.grid(row=1, column=1)
    ...
    moreformats.set(binBool(abConfig.get("moreformats")))
    flacexport.set(binBool(abConfig.get("flacexport")))
    remdndpath.set(binBool(abConfig.get("remdndpath")))
    ...
    if AB_DISABLE_LIBROSA and not AB_USE_ALT_AUBIO: autoonsdet.set(0)

    updateWindow()
    applyWindowStyle(window)
    updFormats(init=True)
