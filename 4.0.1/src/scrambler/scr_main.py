...
from slicing import convert_onsets
...
def calculate_loop_position(cont_length: int, segment: SegmentInfo, state: ScramblerState):
    ...
    if state.loop_position >= state.audio.length:
        state.loop_position %= state.audio.length
        print("Audio loop restart,", state.loop_position)
...
def prepare_framing(state: ScramblerState):
    ...
    if config.framing_reverse_order:
        frames = frames[::-1]
...
def scramble_step(state: ScramblerState, config: ScramblerConfig, qc):
    ...
    mov_length = math.ceil(mov_length * segment_info.speed)
...
def scramble(state: ScramblerState, gui_state, com_progress, com_retry):
    config = state.config
    qc = state.audio.length_ms_to_samp
