...
from scrambler.segment import FrameInfo, calc_segment_end_pos
...
def quantize_length(state: ScramblerState, segment: SegmentInfo):
    config = state.config
    segment_end = calc_segment_end_pos(segment)
    ...
    segment.duration = round1((segment_end - segment.begin) / segment.speed)
...
def fix_sustain_cross(state: ScramblerState, segment: SegmentInfo):
    ...
    segment.sustain_c = max(0, segment.sustain_c)
