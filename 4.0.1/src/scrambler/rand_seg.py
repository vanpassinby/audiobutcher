...
def random_sustain(state: ScramblerState, segment: SegmentInfo, force_subconfig=-1):
    qc = state.audio.length_ms_to_samp
    config = state.config
    subconfig_id = state.config.sustain_weights.get() if force_subconfig < 0 else force_subconfig
...
def gen_random_segment(state: ScramblerState) -> SegmentInfo:
    ...
    if config.abpl_enabled and len(config.abpl_script[0]) > 0:
        script, double_indexing, pre_jump_idx = config.abpl_script
        abpl_state = ScriptState(script, double_indexing, segment, state,
                                 pre_jump_index=pre_jump_idx)
