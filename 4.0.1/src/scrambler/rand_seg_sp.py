...
def random_sp_skip(state: ScramblerState, segment: SegmentInfo):
    ...
    if skip_direction == 1 or (skip_direction == 2 and RandChance().get()):
        skip_size *= -1

    segment.begin = (segment.begin + skip_size) % state.audio.length
...
def random_sp_loop(state: ScramblerState, segment: SegmentInfo):
    ...
    if config.framing and state.loop_frames:
        ...
        segment.begin = state.loop_position % state.audio.length
...
def random_sp_pattern(state: ScramblerState, segment: SegmentInfo):
    ...
    if config.framing and state.loop_frames and config.framing_force_for_pattern.get():
        ...
        pos_margin = math.ceil(segment.duration * segment.speed) if config.duration_priority else 1
        max_position = state.audio.length - pos_margin
