...
class SegmentInfo:
    is_pause = is_avgstart = is_in_loop = from_mem = False
    begin = duration = speed = repeats = intro_loop = None
    volume = reverse1 = reverse2 = None
    sustain = sustain_exact = sustain_c = sustain_shift = sustain_portion = None
    fadein = fadein_cut = fadeout = fadeout_cut = None
    preroll_fadein = crossfade = k_speed = None
...
def calculate_begin_end(info: SegmentInfo, crossfade: int, audio_length: int):
    ...
    segment_end = calc_segment_end_pos(info)
    max_crossfade = max(crossfade, info.crossfade * (info.repeats > 0))
    cross_shift = round(max_crossfade * info.speed)
