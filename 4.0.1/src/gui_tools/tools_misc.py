...
def audio_length_str(len_samples: int, sample_rate: int, in_seconds=True):
    ...
    if in_seconds:
        sec_txt = loc["meta"]["time"]["sec_short"]
        return f"{length}{sec_txt}"
    elif length > 3600:
...
def def_error(action):
    return loc["messages"]["err_unexpected"].format(
        ACTION=loc["messages"]["err_unexpected_actions"].get(action, action)
    )
...
def apply_window_style(window, resize_w=False, resize_h=False):
    ...
    if not AB_DISABLE_ICON:
        icon = PhotoImage(data=meta.icon_b64, format="png")
...
def get_audio_path(window):
    ...
    if window.state.get_pref_var("show_ffmpeg_formats"):
        file_types = fts.ext_audio_import_more
    else:
        file_types = fts.ext_audio_import
