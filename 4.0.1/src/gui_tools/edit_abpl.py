...
from abpl.lib import library, misc_kw
...
def configure_text_tags(text_widget):
    ...
    text_widget.tag_config("misc_kw", font=font)
...
def highlight_line(text_widget, line_number):
    ...
    if PATTERN is None:
        words_pattern = "|".join(re.escape(keyword) for keyword in library.keys() | misc_kw)
    ...
    for match in re.finditer(PATTERN, line_text):
        ...
        the_word = line_text[match.start(1) : match.end(1)]
        if the_word in library:
            text_widget.tag_add("keyword", s, e)
        else:
            text_widget.tag_add("misc_kw", s, e)
...
def edit_abpl(window: "MainWindow"):
    ...
    root.title(loc["tab_tweaks"]["b_abpl_edit"])
    ...
    high_keyword = CCheckbox(root, text=loc["tab_tweaks"]["x_abpl_highlight"])
    ...
    configure_text_tags(edit_field)
    on_key_release()
