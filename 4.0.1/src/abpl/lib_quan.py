...
from scrambler.scr_tools import quantize, quantize_length


def quan_config(state: ScriptState):
    op_name = state.read()
    op_value = read_command(state)
    state.quan_options[op_name] = op_value
...
def cmd_quantize(direction: int):
    def apply_quantization(state: ScriptState):
        ...
        if state.scr_state.config.quan_mode == 1:
            if bool(state.quan_options.get("use_alt", 0)) and state.scr_state.slices_alt:
...
misc_kw = {"use_alt", "min_pos", "max_pos"}
