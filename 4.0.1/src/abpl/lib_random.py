...
from scrambler.rand_seg import random_sustain
...
def random_number(mode):
    def generate(state: ScriptState):
        ...
        return RandNumber(mode, val1, val2).get(force_positive=state.random_positive)
...
def rand_sustain(state: ScriptState):
    index = read_command(state)
    random_sustain(state.scr_state, state.segment, index)


def random_positive_on(state: ScriptState):
    state.random_positive = True


def random_positive_off(state: ScriptState):
    state.random_positive = False
...
library = {
    ...
    "ab_random_sustain": rand_sustain,

    "random_positive_on": random_positive_on,
    "random_positive_off": random_positive_off
}

misc_kw = {"auto", "in_loop", "avgstart", "pattern"}
