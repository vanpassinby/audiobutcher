...
class SustainVariant:
    cb_sustain_portion = BiDict(["total", "n_portion", "p_portion", "p_segment", "p_fadeout"])

    def __init__(self, parent):
        self.root = ttk.Frame(parent, padding=5)
        self.sec_mode_ref = None
        m_loc = loc["tab_sustain"]

        self.u_menu = tk.Menu(self.root, tearoff=False)
        self.u_menu.add_command(label=m_loc["preset_menu"]["mirror"], command=lambda: self.set_preset(1))
        self.u_menu.add_command(label=m_loc["preset_menu"]["mirror_full"], command=lambda: self.set_preset(2))
        self.u_menu.add_command(label=m_loc["preset_menu"]["mirror_asym"], command=lambda: self.set_preset(3))
        self.u_menu.add_command(label=m_loc["preset_menu"]["fadeout"], command=lambda: self.set_preset(4))
        self.u_menu.add_separator()
        self.u_menu.add_command(label=m_loc["preset_menu"]["clear"], command=lambda: self.set_preset(0))
        ...
        self.l_length = ttk.Label(self.root, text=m_loc["l_length"])
        self.e_length = CDistribution(self.root)
        self.l_length_mode = ttk.Label(self.root, text=m_loc["l_length_mode"])
        self.c_length_mode = YCombobox(self.root, width=loc["_geometry"]["cb_sustain_length_mode"], state="readonly",
                                       items=self.cb_sustain_portion, loc_part=m_loc["c_length_mode"])
        self.x_length_exact = CCheckbox(self.root, m_loc["x_length_exact"])
        ...
        self.l_portion_length = ttk.Label(self.root, text=m_loc["l_portion_length"])
        self.e_portion_length = CDistribution(self.root)
        self.x_portion_proportional = CCheckbox(self.root, m_loc["x_portion_proportional"])
        self.l_portion_minimum = ttk.Label(self.root, text=m_loc["l_portion_minimum"])
        ...
        self.x_allow_quan = CCheckbox(self.root, m_loc["x_allow_quan"])
        self.l_shift_chance = ttk.Label(self.root, text=m_loc["l_shift_chance"])
        self.e_shift_chance = CChance(self.root)
        self.l_reverse_chance = ttk.Label(self.root, text=m_loc["l_reverse_chance"])
        self.e_reverse_chance = CChance(self.root)
        self.l_consec_chance = ttk.Label(self.root, text=m_loc["l_consec_chance"])
    ...
    def set_preset(self, idx):
        ...
        if idx == 0:
            self.open_preset(0, PresetOpen())

        elif idx == 1:  # Mirror
            self.e_length.set("0 1 1")
            self.c_length_mode.set("n_portion")
            ...
            self.c_length_mode.set("p_fadeout")
            self.x_length_exact.set(1)
    ...
    def open_preset(self, ident, preset: PresetOpen):
        batch = [
            ...
            [self.c_length_mode, f"sustain{ident}_length_mode", "n_portion"],
    ...
    def save_preset(self, ident, preset: PresetSave):
        ...
        preset.add(f"sustain{ident}_length_mode", self.c_length_mode.get_str())
...
class TabSustain:
    def __init__(self, parent):
        ...
        self.l_chance = ttk.Label(self.root, text=m_loc["l_chance"])
        self.e_chance = CChance(self.root)
        self.l_weights = ttk.Label(self.root, text=m_loc["l_weights"])
        ...
        self.f_variants.add(self.f_var1.root, text=m_loc["t_variant"].format(N_VAR=1))
        self.f_variants.add(self.f_var2.root, text=m_loc["t_variant"].format(N_VAR=2))
        self.f_variants.add(self.f_var3.root, text=m_loc["t_variant"].format(N_VAR=3))
        self.f_variants.add(self.f_var4.root, text=m_loc["t_variant"].format(N_VAR=4))
        self.f_variants.add(self.f_var5.root, text=m_loc["t_variant"].format(N_VAR=5))
