...
from gui_tools.edit_onsets import edit_onsets


class SubShift:
    def __init__(self, parent):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_shift"], padding=3)

        self.l_deviation = ttk.Label(self.root, text=m_loc["l_shift_deviation"])
        self.e_deviation = CDistribution(self.root)
        self.l_chance = ttk.Label(self.root, text=m_loc["l_shift_chance"])
        ...
        self.l_dev_direction = ttk.Label(line2, text=m_loc["l_shift_dev_direction"])
        self.e_dev_direction = CWeights(line2, 2)
        self.x_dev_proportional = CCheckbox(line2, text=m_loc["x_shift_dev_proportional"])
...
class SubFadeCutoff:
    def __init__(self, parent):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_fade"], padding=3)

        self.l_fade_in_cut_dist = ttk.Label(self.root, text=m_loc["l_fade_in_cut_dist"])
        self.e_fade_in_cut_dist = CDistribution(self.root)
        self.l_fade_in_cut_chance = ttk.Label(self.root, text=m_loc["l_fade_in_cut_chance"])
        ...
        self.l_fade_out_cut_dist = ttk.Label(self.root, text=m_loc["l_fade_out_cut_dist"])
        self.e_fade_out_cut_dist = CDistribution(self.root)
        self.l_fade_out_cut_chance = ttk.Label(self.root, text=m_loc["l_fade_out_cut_chance"])
...
class SubVolMute:
    def __init__(self, parent):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_mute"], padding=3)

        self.l_mute_chance = ttk.Label(self.root, text=m_loc["l_mute_chance"])
        self.e_mute_chance = CChance(self.root)
        self.l_mute_to_pause = ttk.Label(self.root, text=m_loc["l_mute_to_pause"], anchor="center")
...
class SubVolume:
    def __init__(self, parent):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_volume"], padding=3)

        self.l_alt_chance = ttk.Label(self.root, text=m_loc["l_volume_chance"])
        ...
        self.l_change = ttk.Label(self.root, text=m_loc["l_volume_change"])
        ...
        self.l_direction = ttk.Label(self.root, text=m_loc["l_vol_direction"])
        self.l_direction_hint = ttk.Label(self.root, text=m_loc["l_vol_direction_hint"])
...
class SubIntroLoop:
    def __init__(self, parent):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_intro_loop"], padding=3)

        self.l_intro_loop_length = ttk.Label(self.root, text=m_loc["l_intro_loop_length"])
        ...
        self.l_intro_loop_chance = ttk.Label(self.root, text=m_loc["l_intro_loop_chance"])
...
class SubQuanPlacement:
    cb_place_quan = BiDict(["disabled", "by_fixed_step", "to_onsets"])

    def __init__(self, parent, window):
        m_loc = loc["tab_misc"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_quan_placement"], padding=3)
        self.onset_data = Onsets([], 1000)
        ref_window = weakref.ref(window)

        self.c_mode = YCombobox(self.root, state="readonly", items=self.cb_place_quan,
                                loc_part=m_loc["c_quan_placement_mode"])
        self.l_step = ttk.Label(self.root, text=m_loc["l_quan_placement_step"])
        self.e_step = XEntry(self.root, width=6)
        self.l_strength = ttk.Label(self.root, text=m_loc["l_quan_placement_strength"])
        self.e_strength = CChance(self.root, ent_w=6)
        self.b_edit_onsets = ttk.Button(self.root, text=m_loc["b_quan_placement_edit_onsets"])
        ...
        self.b_edit_onsets.configure(command=lambda: edit_onsets(ref_window(), self.onset_data))
    ...
    def save_preset(self, preset: PresetSave):
        preset.add("quan_place_mode", self.c_mode.get_str())
...
class TabMisc:
    def __init__(self, parent, window):
        ...
        self.intro_loop.root.grid(row=0, column=1, padx=4, sticky="nsw")
