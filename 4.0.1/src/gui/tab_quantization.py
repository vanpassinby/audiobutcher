...
class SubQuanMain:
    cb_quan_modes = BiDict(["none", "slices", "bpm"])
    cb_quan_directions = BiDict(["closest", "back", "forw"])
    cb_quan_directions_eq = BiDict(["closest", "back", "forw", "equal"])
    cb_quan_directions_auto = BiDict(["closest", "back", "forw", "auto"])

    def __init__(self, parent):
        m_loc = loc["tab_quantization"]
        cb_quan_dir_w = loc["_geometry"]["cb_quan_direction"]
        cb_quan_dir_loc = m_loc["c_quan_directions"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_quan"], padding=3)

        self.l_quan_mode = ttk.Label(self.root, text=m_loc["l_quan_mode"])
        self.c_quan_mode = YCombobox(self.root, width=loc["_geometry"]["cb_quan_mode"], state="readonly",
                                     items=self.cb_quan_modes, loc_part=m_loc["c_quan_mode"])
        self.root.grid_columnconfigure(1, weight=1)
        self.l_quan_mode.grid(row=0, column=0, sticky="w")
        self.c_quan_mode.grid(row=0, column=2, sticky="e")

        self.l_quan_bpm = ttk.Label(self.root, text=m_loc["l_quan_bpm"])
        ...
        frame = ttk.LabelFrame(self.root, text=m_loc["f_quan_chances"], padding=3)
        frame.grid(row=2, column=0, columnspan=3)
        ttk.Label(frame).grid(row=0, column=2)

        self.l_quan_pattern = ttk.Label(frame, text=m_loc["l_quan_pattern"])
        self.e_quan_pattern = CChance(frame)
        self.c_quan_pattern_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                            items=self.cb_quan_directions_eq, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_ast = ttk.Label(frame, text=m_loc["l_quan_ast"])
        self.e_quan_ast = CChance(frame)
        self.c_quan_ast_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                        items=self.cb_quan_directions_auto, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_loop = ttk.Label(frame, text=m_loc["l_quan_loop"])
        self.e_quan_loop = CChance(frame)
        self.c_quan_loop_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                         items=self.cb_quan_directions, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_skip = ttk.Label(frame, text=m_loc["l_quan_skip"])
        self.e_quan_skip = CChance(frame)
        self.c_quan_skip_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                         items=self.cb_quan_directions_auto, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_duration = ttk.Label(frame, text=m_loc["l_quan_duration"])
        self.e_quan_duration = CChance(frame)
        self.c_quan_duration_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                             items=self.cb_quan_directions, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_sustain = ttk.Label(frame, text=m_loc["l_quan_sustain"])
        self.e_quan_sustain = CChance(frame)
        self.c_quan_sustain_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                            items=self.cb_quan_directions, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_frame = ttk.Label(frame, text=m_loc["l_quan_frame"])
        self.e_quan_frame = CChance(frame)
        self.c_quan_frame_dir = YCombobox(frame, width=cb_quan_dir_w, state="readonly",
                                          items=self.cb_quan_directions, loc_part=cb_quan_dir_loc)
        ...
        self.l_quan_alt_slices = ttk.Label(frame, text=m_loc["l_quan_alt_slices"])
    ...
    def save_preset(self, preset: PresetSave):
        preset.add("quan_mode", self.c_quan_mode.get_str())
        preset.add("quan_bpm", self.e_quan_bpm.get())
        preset.add("quan_pattern", self.e_quan_pattern.get_str())
        preset.add("quan_pattern_dir", self.c_quan_pattern_dir.get_str())
        preset.add("quan_ast", self.e_quan_ast.get_str())
        preset.add("quan_ast_dir", self.c_quan_ast_dir.get_str())
        preset.add("quan_loop", self.e_quan_loop.get_str())
        preset.add("quan_loop_dir", self.c_quan_loop_dir.get_str())
        preset.add("quan_skip", self.e_quan_skip.get_str())
        preset.add("quan_skip_dir", self.c_quan_skip_dir.get_str())
        preset.add("quan_duration", self.e_quan_duration.get_str())
        preset.add("quan_duration_dir", self.c_quan_duration_dir.get_str())
        preset.add("quan_sustain", self.e_quan_sustain.get_str())
        preset.add("quan_sustain_dir", self.c_quan_sustain_dir.get_str())
        preset.add("quan_frame", self.e_quan_frame.get_str())
        preset.add("quan_frame_dir", self.c_quan_frame_dir.get_str())
...
class SubQuanAlt:
    def __init__(self, parent):
        m_loc = loc["tab_quantization"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_quan_alt"], padding=3)

        self.x_quan_alt_pattern = CCheckbox(self.root, text=m_loc["x_quan_alt_pattern"])
        self.x_quan_alt_ast = CCheckbox(self.root, text=m_loc["x_quan_alt_ast"])
        self.x_quan_alt_loop = CCheckbox(self.root, text=m_loc["x_quan_alt_loop"])
        self.x_quan_alt_skip = CCheckbox(self.root, text=m_loc["x_quan_alt_skip"])
        self.x_quan_alt_duration = CCheckbox(self.root, text=m_loc["x_quan_alt_duration"])
        self.x_quan_alt_portion = CCheckbox(self.root, text=m_loc["x_quan_alt_portion"])
        self.x_quan_alt_frame = CCheckbox(self.root, text=m_loc["x_quan_alt_frame"])
