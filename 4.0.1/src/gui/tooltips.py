...
class SimpleAccessH:
    def __init__(self, data: dict):
        self.data = {}
        self_data = object.__getattribute__(self, "data")

        for sub_dict in data.values():
            # WARNING: This code assumes every item of this dict is also a dict
            self_data |= sub_dict

    def __getattribute__(self, item):
        return object.__getattribute__(self, "data").get(item, item)
...
def apply_tooltips(window: "MainWindow"):
    ...
    def a_basic_tab():
        ...
        tt(tab_basic.e_duration, rand_dist)
        ...
        tt(tab_basic.e_pause, rand_dist)
        ...
        tt(tab_basic.e_crossfade, rand_dist)
        ...
        tt(tab_basic.e_fadein, rand_dist)
        ...
        tt(tab_basic.e_fadeout, rand_dist)
        tt(tab_basic.l_fadeout_chance, h.chance)
        tt(tab_basic.e_fadeout_chance, h.chance)

        tt(tab_basic.l_fade_only_into_pauses, h.fade_only_into_pauses)
        tt(tab_basic.e_fade_only_into_pauses, h.fade_only_into_pauses)
        tt(tab_basic.x_fade_out_perc_note, h.fade_out_from_slice)

        tt(tab_basic.l_repeat, h.repeat)
        tt(tab_basic.e_repeat, rand_dist)
        ...
        tt(tab_basic.e_reappear, rand_dist_units)
        ...
        tt(fr_speed.l_speed_variations, h.speed_alt_list.format(SPACE_SEP=h.space_sep))
        tt(fr_speed.e_speed_variations, h.speed_alt_list.format(SPACE_SEP=h.space_sep))
    ...
    def a_sustain():
        ...
        for sus_var in (tab_sustain.f_var1, tab_sustain.f_var2, tab_sustain.f_var3,
                        tab_sustain.f_var4, tab_sustain.f_var5):

            tt(sus_var.l_crossfade, h.sus_cross)
            tt(sus_var.e_crossfade, h.sus_cross)
            tt(sus_var.l_length, h.sus_length)
            tt(sus_var.e_length, rand_dist)
            ...
            tt(sus_var.e_portion_length, rand_dist)
    ...
    def a_pattern():
        ...
        tt(sub_pattern.l_break_skips, h.loop_can_break.format(SEGMENT=h.loop_break_skip_repl)+h.loop_break_skip_add)
        tt(sub_pattern.e_break_skips, h.loop_can_break.format(SEGMENT=h.loop_break_skip_repl)+h.loop_break_skip_add)
        tt(sub_pattern.l_break_pattern, h.loop_can_break.format(SEGMENT=h.loop_break_pattern_repl))
        tt(sub_pattern.e_break_pattern, h.loop_can_break.format(SEGMENT=h.loop_break_pattern_repl))
        tt(sub_pattern.l_break_avgstart, h.loop_can_break.format(SEGMENT=h.loop_break_ast_repl)+h.loop_break_skip_add)
        tt(sub_pattern.e_break_avgstart, h.loop_can_break.format(SEGMENT=h.loop_break_ast_add)+h.loop_break_skip_add)
        ...
        for skip_var in (sub_skip.f_forw, sub_skip.f_back, sub_skip.f_rand):
            tt(skip_var.l_weight, h.skip_weight)
            tt(skip_var.e_weight, h.skip_weight)
        ...
        tt(sub_ast.l_times, h.ast_list.format(SPACE_SEP=h.space_sep))
        tt(sub_ast.e_times, h.ast_list.format(SPACE_SEP=h.space_sep))
    ...
    def a_framing():
        ...
        tt(tab_framing.e_frame_size, rand_dist)
        ...
        tt(sub_env.e_attack_dist, rand_dist)
        tt(sub_env.x_hold, h.fr_env_hold)
        tt(sub_env.e_hold_dist, rand_dist)
        tt(sub_env.x_decay, h.fr_env_decay)
        tt(sub_env.e_decay_dist, rand_dist)
    ...
    def a_misc():
        ...
        tt(sub_shift.e_deviation, rand_dist)
        ...
        tt(sub_cut.e_fade_in_cut_dist, rand_dist_units)
        ...
        tt(sub_cut.e_fade_out_cut_dist, rand_dist_units)
        ...
        tt(sub_vol.e_change, rand_dist_units)
    ...
    def a_tweak():
        ...
        tt(sub_consec.x_volume_pause_is_mute, h.pause_is_mute)
    ...
    h = SimpleAccessH(loc["tooltips"])
    rand_dist = h.rand_dist + h.rand_dist_lognorm
    rand_dist_units = rand_dist
