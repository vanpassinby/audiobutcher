...
class SubFramingGeneral:
    def __init__(self, parent):
        m_loc = loc["tab_framing"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_general"], padding=3)

        self.x_enabled = CCheckbox(self.root, m_loc["x_framing_enable"])
        self.l_frame_size = ttk.Label(self.root, text=m_loc["l_frame_size"])
        self.e_frame_size = CDistribution(self.root)
        self.x_use_seed = CCheckbox(self.root, m_loc["x_use_seed"])
        self.e_seed = XEntry(self.root)
        self.l_speed_ratio = ttk.Label(self.root, text=m_loc["l_speed_ratio"])
        self.e_speed_ratio = XEntry(self.root)
        self.x_alt_reverse = CCheckbox(self.root, text=m_loc["x_alt_reverse"])
        self.x_reverse_order = CCheckbox(self.root, text=m_loc["x_reverse_order"])
        ...
        self.x_reverse_order.root.grid(row=5, column=0, columnspan=2, sticky="w")

        frame_force = ttk.Frame(self.root)
        self.l_force_duration = ttk.Label(frame_force, text=m_loc["l_force_duration"])
        self.e_force_duration = CChance(frame_force)
        self.l_force_for_pattern = ttk.Label(frame_force, text=m_loc["l_force_for_pattern"])
        self.e_force_for_pattern = CChance(frame_force)
        self.l_force_for_ast = ttk.Label(frame_force, text=m_loc["l_force_for_ast"])
        self.e_force_for_ast = CChance(frame_force)

        ttk.Label(self.root).grid(row=6, column=0)
        frame_force.grid(row=7, column=0, columnspan=2, sticky="w")
    ...
    def apply_config(self, config: ScramblerConfig):
        ...
        config.framing_reverse_order = self.x_reverse_order.get()
    ...
    def open_preset(self, preset: PresetOpen):
        batch = [
            ...
            [self.x_reverse_order, "framing_reverse_order", "0"],
    ...
    def save_preset(self, preset: PresetSave):
        ...
        preset.add("framing_reverse_order", self.x_reverse_order.get_str())
...
class SubFramingEnvelope:
    def __init__(self, parent):
        m_loc = loc["tab_framing"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_envelope"], padding=3)

        self.x_attack = CCheckbox(self.root, text=m_loc["x_env_attack"])
        self.e_attack_dist = CDistribution(self.root)
        self.x_hold = CCheckbox(self.root, text=m_loc["x_env_hold"])
        self.e_hold_dist = CDistribution(self.root)
        self.x_decay = CCheckbox(self.root, text=m_loc["x_env_decay"])
        self.e_decay_dist = CDistribution(self.root)
        self.x_crossfade_endless = CCheckbox(self.root, text=m_loc["x_env_crossfade_endless"])
...
class SubFramingAltLength:
    def __init__(self, parent):
        m_loc = loc["tab_framing"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_length"], padding=3)

        self.l_soft = ttk.Label(self.root, text=m_loc["l_length_scale"])
        self.e_soft = CChance(self.root, ent_w=4)
        self.l_hard = ttk.Label(self.root, text=m_loc["l_length_cutoff"])
...
class SubFramingSimplify:
    def __init__(self, parent):
        m_loc = loc["tab_framing"]
        self.root = ttk.LabelFrame(parent, text=m_loc["f_simplify"], padding=3)

        self.x_enable = CCheckbox(self.root, text=m_loc["x_simplify_enable"])
        self.l_step = ttk.Label(self.root, text=m_loc["l_simplify_step"])
        self.e_step = XEntry(self.root, width=6)
        self.l_severity = ttk.Label(self.root, text=m_loc["l_simplify_severity"])
...
class TabFraming:
    ...
    def update(self):
        if self.general.x_enabled.get():
            ...
            self.general.x_reverse_order.root.configure(state="normal")
            ...
            self.general.x_reverse_order.root.configure(state="disabled")
