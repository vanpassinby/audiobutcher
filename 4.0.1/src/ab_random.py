...
class RandNumber:
    ...
    def get(self, force_positive=True):
        if self.mode == RD.uniform:
            value = random.uniform(self.value1, self.value2)
            if force_positive:
                value = abs(value)
            return value
            ...
            while True:
                value = random.gauss(self.value1, self.value2)
                if value >= 0 or not force_positive:
                    break
            return value

        elif self.mode == RD.gauss_c:
            min_val = self.value1 - abs(self.value2)
            max_val = self.value1 + abs(self.value2)

            if force_positive:
                min_val = max(0, min_val)

            if max_val < 0 and force_positive:
